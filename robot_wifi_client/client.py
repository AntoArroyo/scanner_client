...
class WifiScannerNode(Node):
    def __init__(self, device_id, server_ip, map_name='LAB', timer=5.0, use_median=True):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if est_pos:
                    ...
                    # Decide which position to publish: raw or filtered
                    if self.use_median and self.position_buffer:
                        columnas = list(zip(*self.position_buffer))
                        filtered_pos = tuple(statistics.median(col) for col in columnas)
                        self.get_logger().info(f"Buffer median result: {filtered_pos}")
                        self.publish_marker(filtered_pos, ns='estimated_position_median')
                    else:
                        self.publish_marker(tupla)
            else:
